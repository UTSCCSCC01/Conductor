...
app.use(express.json()); //middleware for the server to read JSON for POST request
/*
...
*/
app.get('/dashboard/marketplace/:buid', (req, res) => {        //temporary url. get information about the bot name in marketplace/:name
    MongoClient.connect(url, function (err, db) {     //connect to the mongoDB url provided above (1)
        ...
        var database = db.db("Marketplace");           // storing bot_infoDB database in our url
        database.collection("Bots").findOne({          // finding bot object by the name in the collection "bot" in bot_infoDB
            buid: req.params.buid
        },
            function (err, result) {
                if (err) throw err;                   // if error in fetching the data, throw err
                if (!result) return res.status(404).send(`No bot with buid ${req.params.buid} found`); // if no bot exists, send 404
...
app.get('/dashboard/marketplace/:buid/reviews', (req, res) => {        //temporary url. get information about the bot name in marketplace/:name
    MongoClient.connect(url, function (err, db) {     //connect to the mongoDB url provided above (1)
        if (err) throw err;                           // if in case of an error, we give an error.
        var database = db.db("Marketplace");           // storing bot_infoDB database in our url
        database.collection("Reviews").find({          // finding bot object by the name in the collection "bot" in bot_infoDB
            buid: req.params.buid
        },
            function (err, result) {
                if (err) throw err;                   // if error in fetching the data, throw err
...
*/
app.post('/dashboard/botbuilder/upload', (req, res) => {       // add bot to the database
